...
void SWorkspaceData::recalculate(bool forceInstant) {
    ...
    for (size_t i = 0; i < columns.size(); ++i) {
        ...
        for (const auto& WINDOW : COL->windowDatas) {
            WINDOW->layoutBox =
                CBox{currentLeft, currentTop, ITEM_WIDTH, WINDOW->windowSize * USABLE.h}.translate(PMONITOR->m_position + Vector2D{PMONITOR->m_reservedArea.left(), PMONITOR->m_reservedArea.top()} + Vector2D{-cameraLeft, 0.0});
...
void CScrollingLayout::applyNodeDataToWindow(SP<SScrollingWindowData> data, bool force, bool hasWindowsRight, bool hasWindowsLeft) {
    ...
    const bool DISPLAYTOP    = STICKS(data->layoutBox.y, PMONITOR->m_position.y + PMONITOR->m_reservedArea.top());
    const bool DISPLAYBOTTOM = STICKS(data->layoutBox.y + data->layoutBox.h, PMONITOR->m_position.y + PMONITOR->m_size.y - PMONITOR->m_reservedArea.bottom());
    ...
    if (EFFECTIVE_MODE == FSMODE_NONE) {
        ...
        if (EFFECTIVE_MODE == FSMODE_FULLSCREEN) {
            ...
            *pWindow->m_realSize     = PMONITOR->m_size;
            ...
            fakeNode->layoutBox = {PMONITOR->m_position + Vector2D{PMONITOR->m_reservedArea.left(), PMONITOR->m_reservedArea.top()}, PMONITOR->m_size - Vector2D{PMONITOR->m_reservedArea.left(), PMONITOR->m_reservedArea.top()} - Vector2D{PMONITOR->m_reservedArea.right(), PMONITOR->m_reservedArea.bottom()}};
...
CBox CScrollingLayout::usableAreaFor(PHLMONITOR m) {
    return CBox{Vector2D{m->m_reservedArea.left(), m->m_reservedArea.top()}, m->m_size - Vector2D{m->m_reservedArea.left(), m->m_reservedArea.top()} - Vector2D{m->m_reservedArea.right(), m->m_reservedArea.bottom()}};
}
